...
function fromExisting(l: ExistingLine): Line {
  return {
    key: newKey(),
...
function emptyLine(): Line {
  return { key: newKey(), productId: '', name: '', description: '', quantity: 1, unitPrice: 0, vatRate: 21 }
}

export default function QuoteEditForm({ quoteId, initialData, products }: Props) {
    ...
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={s.card}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          ...
          <button type="button" onClick={addLine} style={{ padding: '5px 12px', background: 'var(--bg-elevated)', border: '1px solid var(--border-strong)', borderRadius: 6, color: 'var(--text-primary)', fontSize: 12.5, cursor: 'pointer' }}>
            + Regel toevoegen
          </button>
        </div>
        ...
        {lines.map((line) => {
          ...
          return (
            <div key={line.key} style={s.lineRow}>
              ...
              />
              <select value={line.vatRate} onChange={(e) => updateLine(line.key, { vatRate: parseFloat(e.target.value) })}
              ...
              <button type="button" onClick={() => removeLine(line.key)} disabled={lines.length === 1}
                style={{ width: 28, height: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'transparent', border: 'none', color: 'var(--text-tertiary)', cursor: lines.length === 1 ? 'not-allowed' : 'pointer', borderRadius: 4, flexShrink: 0, opacity: lines.length === 1 ? 0.3 : 1 }}
              >
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        <button
          type="submit"
          disabled={saving}
          style={{ padding: '8px 18px', background: saving ? 'var(--bg-elevated)' : 'var(--accent)', color: '#fff', border: 'none', borderRadius: 8, fontSize: 13.5, fontWeight: 500, cursor: saving ? 'not-allowed' : 'pointer' }}
        >
        ...
        <button
          type="button"
          onClick={() => router.back()}
          style={{ padding: '8px 14px', background: 'var(--bg-elevated)', border: '1px solid var(--border-strong)', borderRadius: 8, color: 'var(--text-primary)', fontSize: 13.5, cursor: 'pointer' }}
        >
          Annuleren
        </button>
      </div>
...
const styles = {
  card: { background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 12, padding: 20 } as React.CSSProperties,
  cardTitle: { fontWeight: 600, fontSize: 13.5, marginBottom: 16 } as React.CSSProperties,
  field: { display: 'flex', flexDirection: 'column', gap: 6 } as React.CSSProperties,
  label: { fontSize: 12.5, fontWeight: 500, color: 'var(--text-secondary)' } as React.CSSProperties,
  input: { background: 'var(--bg-elevated)', border: '1px solid var(--border-strong)', borderRadius: 8, padding: '8px 12px', color: 'var(--text-primary)', fontSize: 13.5, outline: 'none', width: '100%', fontFamily: 'var(--font-sans)', boxSizing: 'border-box' } as React.CSSProperties,
  lineHeader: { display: 'flex', gap: 10, alignItems: 'center', padding: '6px 0 10px', borderBottom: '1px solid var(--border)', marginBottom: 10, fontSize: 11, fontWeight: 600, color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.05em' } as React.CSSProperties,
  lineRow: { display: 'flex', gap: 10, alignItems: 'flex-start', padding: '8px 0', borderBottom: '1px solid var(--border)' } as React.CSSProperties,
}
